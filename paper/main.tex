...
\title{Predict, Then Measure: An Open, Reproducible\\
Design-Space-to-RTL Case Study for a Depthwise-Separable\\
CNN Keyword-Spotting Accelerator}
...
by pipelining. Along the way we correct a common depthwise-modeling pitfall that
...
fusion ($-23\%$) and int8$\rightarrow$int4 precision ($-50\%$ energy for a
measured \approxs$4.5$-point top-1 accuracy drop). The entire flow is
...
here, so full int4 would cost more accuracy; and this is naive post-training
quantization---quantization-aware training typically recovers most of the int4
gap. The precision knob is thus a \emph{measured} accuracy--energy trade rather
than an open assumption.
...
weight quantization halves per-inference energy for a \approxs$4.5$-point top-1
drop. Weight-only PTQ; quantization-aware training would recover much of it.}
\label{fig:accenergy}
\end{figure}
...
\approxs$23\%$ area reduction (accuracy cost not evaluated here).}
\label{fig:precision}
\end{figure}
...
Table~\ref{tab:pvm} reconciles the two estimates. Area is on different nodes, so
a fair comparison scales the 45\,nm estimate to 130\,nm (ideal area scaling,
\approxs$8.35\x$) and notes that synthesis area is pre-place-and-route; applying
a typical utilization correction to the synthesized cell area lands it within
\approxs$1\x$ of the scaled prediction (Fig.~\ref{fig:pvm}). The clock, by
contrast, was optimistic by \approxs$7\x$.

\begin{table}[t]
\caption{Predicted (Accelergy, 45\,nm) vs.\ measured (Sky130, 130\,nm) for the
MAC compute core.}
\label{tab:pvm}
\centering\footnotesize
\begin{tabular}{lcc}
\toprule
 & Area & Clock \\
\midrule
Predicted (45\,nm)         & 71k\,\um$^2$              & 1\,GHz (assumed) \\
\quad scaled to 130\,nm    & \approxs593k\,\um$^2$     & --- \\
Measured (130\,nm synth)   & 369k\,\um$^2$             & 147\,MHz \\
\quad P\&R-adjusted / pipelined & \approxs615k\,\um$^2$ & 319\,MHz \\
\midrule
Agreement                  & within \approxs$1\x$      & $7\x$ off, $2.2\x$ recovered \\
\bottomrule
\end{tabular}
\end{table}
...
Only the compute core is implemented in RTL; the buffers and control are modeled
analytically. Timing is gate-level static timing, not a full place-and-route
sign-off with power extraction. The energy/area tables are 45\,nm while the
implementation is 130\,nm, which is why the study relies on relative ranking and
scales explicitly in the comparison. The int4 result is hardware-only; its
accuracy cost is unmeasured. Finally, a single workload and dataflow family are
studied. Future work is a full OpenLane place-and-route with power sign-off, an
accuracy--energy Pareto for reduced precision, RTL for the buffer and control
path, deeper pipelining toward higher frequency, and additional dataflows.
