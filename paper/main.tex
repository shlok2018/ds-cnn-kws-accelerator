...
\emph{End-to-end.} To confirm the datapath runs the real workload and not just
isolated matmuls, a bit-exact functional model of the array executes the
\emph{complete} DS-CNN---all nine convolutional layers and the FC classifier,
...
So the accelerator does not merely multiply---it correctly spots keywords. The
multi-layer scheduling is software-driven here; a hardware sequencer is future work.
...
adder-limited pipelined clock, dense SRAM-macro buffers and on-board FPGA
bring-up, an accuracy--energy Pareto for reduced precision, and additional
dataflows.
...
$143$\,MHz sign-off at the typical corner) at lower area and power. In the
