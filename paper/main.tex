...
usual ``the accelerator only does the matmuls'' caveat. The entire flow is
...
RTL simulation (bit-exact against the static-quantized golden); the area, timing,
and power sign-off numbers are for the 8$\times$8 compute core we placed and
routed, not the sequencer as a whole. On FPGA that core maps, but the sequencer's
large behavioral buffers do not infer Block RAM without a banked, streamed
datapath---the parallel array writes 64 accumulators and reads 8 operands per
cycle, exceeding a two-port RAM---which we leave to future work.
...
adder-limited pipelined clock; dense SRAM-macro buffers and a Block-RAM-friendly
(banked, streamed) memory datapath so the full sequencer maps to FPGA for
on-board bring-up; an accuracy--energy Pareto for reduced precision; and
