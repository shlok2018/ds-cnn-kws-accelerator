...
by pipelining. A full OpenLane RTL-to-GDS sign-off on sky130 closes the loop: the
design is LVS- and DRC-clean, and the $147$\,MHz gate-level estimate tracks
post-placement \emph{typical} timing to \approxs$6\%$---but multi-corner
\emph{post-route} static timing exposes a further gap, the committed $143$\,MHz
failing at the slow corner ($-6.2$\,ns worst setup) for a true across-corner
sign-off of \approxs$75$\,MHz. Pipelining the MAC and the accumulator read-out
then takes the design from failing to a \emph{completed} $143$\,MHz sign-off
(meeting typical corners; \approxs$103$\,MHz across all corners) at lower area
($0.63{\to}0.47$\,mm$^2$) and \approxs$60\%$ lower power. The cheap pre-layout
numbers thus rank designs and predict area faithfully, while each step toward
real sign-off---multi-corner, then post-route---reveals more of the clock
optimism. Along the way we correct a common depthwise-modeling pitfall that
...
top-1 accuracy loss once the 4-bit weights are fine-tuned). Finally, a
synthesizable multi-layer sequencer runs the \emph{complete} ten-layer DS-CNN
end-to-end in hardware---no software scheduling---bit-exact against a
static-quantized golden and at \approxs$91\%$ top-1 on real speech, retiring the
usual ``the accelerator only does the matmuls'' caveat; after banking its
datapath to Block RAM and time-multiplexing one array and one requant lane across
the layer engines, the whole sequencer places and routes on an ECP5-85 FPGA. The
entire flow is open-source and reproducible.
\end{abstract}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig1_roofline.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\columnwidth]{fig_fusion.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{fig_accuracy_energy.pdf}
...
\emph{Post-layout sign-off.} To test the pre-layout estimates against a real
physical implementation rather than stopping at synthesis, the array---wrapped in
a registered read-out mux so results leave through a narrow port instead of
2\,048 pins---was taken through the full open-source OpenLane RTL-to-GDS flow on
sky130. It routes \emph{DRC-clean} (0 violations, by both Magic and KLayout) and
\emph{LVS-clean} (Netgen: circuits match uniquely), with a post-route
standard-cell area of $0.63$\,mm$^2$ and \approxs$273$\,mW dynamic power at the
typical corner and full activity---the area and power both matching the pre-RTL
predictions. Post-placement typical timing closes at \approxs$139$\,MHz, within
\approxs$6\%$ of the $147$\,MHz gate-level STA, so the cheap estimate faithfully
predicts the \emph{typical} routed clock. Full multi-corner \emph{post-route}
sign-off, however, is sharper: the committed $143$\,MHz \emph{fails} at the slow
(SS) corner with a worst setup slack of $-6.2$\,ns, and the true across-corner
closing frequency is \approxs$75$\,MHz---roughly half the typical estimate. The
register-to-register critical path is not the MAC arithmetic but the single-cycle
$64{:}1{\times}32$-bit accumulator read-out mux.

\emph{Recovering the clock.} Pipelining both the PE (a register between the
multiplier and the accumulate-add) and the read-out mux (the $64{:}1$ select
split into two registered $8{:}1$ stages) targets exactly that path. Through the
same sign-off flow at the $143$\,MHz target, the pipelined top \emph{completes}
where the original flow aborts on timing, meeting timing at the typical and fast
corners; its worst-corner setup slack improves to $-2.7$\,ns, lifting the
all-corner sign-off frequency to \approxs$103$\,MHz while \emph{reducing} cell
area from $0.63$ to $0.47$\,mm$^2$ and dynamic power by \approxs$60\%$ (the
original having spent \approxs$6.7$k buffers losing a setup battle). The new
worst-corner path, verified from the sign-off STA, is the $32$-bit accumulator
adder---which product-pipelining does not shorten---the next lever for higher
frequency.
...
\emph{End-to-end, software model.} To confirm the datapath runs the real workload
and not just isolated matmuls, a bit-exact functional model of the array executes
the \emph{complete} DS-CNN---all nine convolutional layers and the FC classifier,
each lowered to the 8$\times$8 int8 tiled matmul the array implements, with
BatchNorm folded in and int8 requantization between layers---on real MFCC clips.
It reaches \textbf{92.33\%} top-1 accuracy on the 4\,890-clip MLPerf Tiny test
set, matching the float reference (92.2\%) and the measured int8 PTQ (92.3\%).
So the accelerator does not merely multiply---it correctly spots keywords.

\emph{End-to-end, hardware sequencer.} We then move the orchestration itself into
hardware. A synthesizable multi-layer sequencer (\texttt{dscnn\_seq}) walks a
per-layer descriptor table and drives three verified engines---a standard/%
pointwise-convolution engine, a depthwise engine, and the pooling/FC classifier
tail---each composed of a tiling GEMM unit, an integer requantization/activation
lane, and an im2col address generator; feature maps chain layer-to-layer through
ping-pong on-chip buffers, and the terminal FC emits the predicted keyword. Fixed
hardware cannot use the per-clip \emph{dynamic} activation scales of the software
model, so we calibrate one \emph{static} per-tensor scale per layer and fold each
layer to integer requant parameters (a multiplier, shift, and bias). Run on real
MFCC clips, the whole ten-layer network then executes end-to-end in RTL with
\emph{no software scheduling}, bit-exact against the static-quantized software
golden, at \approxs$91\%$ top-1---the accuracy of a fixed-function chip, about a
point below the dynamic scheme.

\emph{Full network on FPGA.} This sequencer is not simulation-only. Mapping it to
fabric requires an FPGA-oriented datapath: the tiling GEMM's operand and result
memories are banked into single-port \emph{synchronous} block RAM (A by row, W and
O by column, and the former 64-wide tile write-back serialized over eight cycles),
which alone takes the core from an un-mappable 64-write-port register blob to
inferred Block RAM; one $8\times8$ MAC array and one requantization lane are then
time-multiplexed across all three engines (only one runs per layer), collapsing
$3\times64$ to $64$ DSPs; and the sequencer's weight and ping-pong feature-map
buffers are likewise banked to block RAM. With these changes the \emph{entire}
\texttt{dscnn\_seq}---every one of the ten layers, still bit-exact against the
static-quantized golden---places and routes on a Lattice ECP5-85 through the open
Yosys\,+\,nextpnr flow, using $97$ DSP multipliers ($62\%$), $68$ block RAMs
($32\%$), and $90\%$ of the LUTs, and closing at \approxs$13$\,MHz. The critical
path is the shared, \emph{unpipelined} requantization multiplier---the natural next
pipelining target, exactly as the read-out mux was for the ASIC. This is the whole
network, not merely the compute core, running on commodity FPGA fabric.

\section{Predicted vs. Measured}
Table~\ref{tab:pvm} reconciles the estimates against synthesis, place-and-route,
and full multi-corner post-route sign-off. Area is on different nodes, so a fair
comparison scales the 45\,nm estimate to 130\,nm (ideal area scaling,
\approxs$8.35\x$); the synthesized bare-array cell area then lands within
\approxs$1\x$ of the scaled prediction (Fig.~\ref{fig:pvm}), and the routed design
(0.63\,mm$^2$, which also includes the read-out mux and clock-tree/buffer/fill
overhead) is consistent with it. Timing is the more telling axis, and it degrades
at each step of rigor: the assumed $1$\,GHz was \approxs$7\x$ optimistic; the
$147$\,MHz gate-level STA then predicted the \approxs$139$\,MHz post-placement
\emph{typical} clock to within \approxs$6\%$; but full multi-corner
\emph{post-route} sign-off shows the committed $143$\,MHz failing at the slow
corner ($-6.2$\,ns setup), for a true across-corner Fmax of \approxs$75$\,MHz---
about half the typical figure. So the cheap pre-layout estimates rank designs and
predict area reliably, and predict the \emph{typical} clock well, but the
worst-corner sign-off clock---the one a product must actually meet---is lower
still, and only the full flow reveals it.

\begin{table}[t]
\caption{Predicted (Accelergy, 45\,nm) vs.\ measured (Sky130, 130\,nm), from
synthesis through full multi-corner post-route sign-off. Area agrees to
\approxs$1\x$; the clock degrades at each step of rigor, and pipelining the MAC
and read-out recovers the all-corner sign-off to \approxs$103$\,MHz.}
\label{tab:pvm}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcc}
...
OpenLane P\&R, post-place       & 0.63\,mm$^2$            & 139\,MHz (typical) \\
\quad post-route sign-off       & LVS/DRC-clean           & \approxs75\,MHz (all-corner) \\
\quad + pipelined read-out      & 0.47\,mm$^2$            & \approxs103\,MHz (all-corner) \\
\midrule
Agreement                       & within \approxs$1\x$    & typ.\ to \approxs$6\%$; sign-off \approxs$2\x$ lower \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_rtl_vs_pred.pdf}
...
applied; but its implicit \emph{timing} assumption must be validated against a
full multi-corner, post-route sign-off and de-rated accordingly, because the
achievable clock is set by the microarchitecture (here, the accumulator read-out
mux, then---once pipelined---the accumulator adder) and by slow-corner derating,
...
\section{A Microcosm of Commercial NPU Design}
Always-on keyword spotting is not a toy workload: it is a shipping on-device
function, and the depthwise-separable network that implements it is representative
of the models mapped to commercial mobile neural accelerators such as the Apple
Neural Engine, Qualcomm Hexagon, and Google Edge TPU. The $8\times8$ int8
weight-stationary core studied here is thus a research-scale instance of a real
product block, and the flow that produced it mirrors the industrial one.
Architectural design-space exploration---Timeloop and Accelergy here
\cite{timeloop,accelergy}, proprietary power/performance/area models inside a
company---steers a microarchitecture that is then captured in RTL, synthesized,
placed and routed, checked for DRC and LVS, statically timed across process
corners, and prototyped on an FPGA, while the network is co-designed with the
hardware through quantization. That closed loop---estimate, commit, implement,
measure, reconcile---is the same one a commercial NPU program runs
\cite{tpu,eyeriss,simba}.

The differences are of \emph{scale and sign-off depth, not of methodology}
(Table~\ref{tab:industrial}). A commercial part is fabricated on a leading-edge
FinFET/GAA node with EUV rather than an open 130\,nm planar PDK; it is built with
commercial EDA (Synopsys, Cadence, Siemens Calibre) from a mix of standard cells,
full-custom blocks, and compiled SRAM rather than open-source standard-cell
auto-place-and-route; it clears a broader sign-off---power integrity (IR-drop,
electromigration), thermal, ESD/latch-up, and aging on top of the DRC, LVS, and
static timing an open RTL-to-GDS flow performs---and is verified by
constrained-random (UVM), formal, and hardware-emulation campaigns rather than
directed testbenches. It is also a full system-on-chip built by a large team at a
multi-million-dollar mask cost, of which a wake-word accelerator is one small
block. None of these alter the \emph{shape} of the design flow; they raise its
node, cost, and rigor. The value of an open study like this one is that it
reproduces that shape end-to-end on free tools, so the methodology---and the
fidelity data attached to it---can be inspected and re-run without a proprietary
back end.

\begin{table}[t]
\caption{This open study vs.\ a commercial mobile-NPU program. The design loop is
shared; node, tooling, verification, sign-off breadth, and scale differ.}
\label{tab:industrial}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lll}
\toprule
 & This work & Commercial mobile NPU \\
\midrule
Node / fab   & Sky130 130\,nm (open) $+$ FPGA   & 3--5\,nm FinFET/GAA, EUV \\
EDA tools    & OpenLane, Yosys, OpenROAD (open) & Synopsys, Cadence, Calibre \\
Design style & Standard-cell auto-P\&R          & $+$ full-custom, compiled SRAM \\
Verification & Directed self-checking TB        & UVM, formal, emulation, FPGA \\
Sign-off     & DRC, LVS, multi-corner STA       & $+$ IR/EM, thermal, ESD, aging \\
Scope / team & One block, one engineer          & Full SoC, large team, \$10M$+$ masks \\
\bottomrule
\end{tabular}}
\end{table}
...
The multi-layer sequencer that runs the whole network is functionally verified in
RTL simulation (bit-exact against the static-quantized golden); the \emph{ASIC}
area, timing, and power sign-off numbers, however, are for the 8$\times$8 compute
core we placed and routed, not the sequencer as a whole. The \emph{FPGA} mapping,
by contrast, now covers the entire sequencer: banking the array and buffer
memories into synchronous Block RAM and time-multiplexing one array/requant lane
across the engines takes the full network from un-mappable---the parallel array
originally wrote 64 accumulators and read 8 operands per cycle, exceeding a
two-port RAM---to a completed ECP5-85 place-and-route, though at a low
\approxs$13$\,MHz set by the unpipelined requantization multiplier and with $90\%$
LUT utilization. It does not fit the smaller ECP5-45 (both DSP and LUT exceed it);
the residual DSPs are per-layer dimension products that would move into the
descriptor, which we leave to future work.
The place-and-route now completes full sign-off---DRC-clean (Magic and KLayout),
LVS-clean (Netgen), and multi-corner post-route STA and power---but two caveats
remain. OpenLane's design-level antenna \emph{check} is skipped (a late-flow
checker crash); antenna-diode insertion still runs and the router reports fewer
than $200$ residual antenna nets, so antenna sign-off specifically is incomplete.
And the reported power is at full switching activity, not the low-duty-cycle
always-on average (per-inference energy remains the meaningful always-on figure).
The energy/area tables are 45\,nm while the implementation is 130\,nm, which is
why the study relies on relative ranking and scales explicitly in the comparison.
The int4 accuracy is measured with weights-only quantization and a short,
CPU-bounded fine-tune; activation quantization and a fuller precision sweep are
future work. Finally, a single workload and dataflow family are studied. Future
work is a narrower or faster accumulator to move past the \approxs$103$\,MHz,
adder-limited pipelined clock; pipelining the shared requantization multiplier to
lift the full-sequencer FPGA clock past its \approxs$13$\,MHz first-cut and taking
the routed bitstream to on-board bring-up; dense SRAM-macro buffers for the ASIC;
an accuracy--energy Pareto for reduced precision; and additional dataflows.
...
signed-off compute core (LVS- and DRC-clean, multi-corner post-route STA and
power) on an open 130\,nm PDK, and reported both sides of the outcome. The area
estimate held to within \approxs$1\x$; the clock, though, degraded at each step
of rigor---\approxs$7\x$ optimistic as an assumed $1$\,GHz, \approxs$139$\,MHz at
post-placement typical, and \approxs$75$\,MHz once multi-corner post-route
sign-off is applied---after which pipelining the compute and the accumulator
read-out recovered the all-corner clock to \approxs$103$\,MHz (a completed
$143$\,MHz sign-off at the typical corner) at lower area and power. Beyond the
core, a synthesizable multi-layer sequencer then runs the \emph{complete}
ten-layer DS-CNN end-to-end in hardware---bit-exact against a static-quantized
golden at \approxs$91\%$ top-1---so keyword spotting happens on-chip rather than
being orchestrated in software. In the
