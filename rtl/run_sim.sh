...
iverilog -g2012 -o /tmp/tb_layer.vvp mac_int8.sv mac_array_8x8.sv gemm_top_bram.sv im2col_gen.sv requant_unit.sv layer_engine.sv tb_layer.sv; vvp /tmp/tb_layer.vvp
echo "== depthwise layer in HW (per-channel conv + requant, all channels) =="
iverilog -g2012 -o /tmp/tb_dw.vvp mac_int8.sv mac_array_8x8.sv gemm_top_bram.sv im2col_gen.sv requant_unit.sv dw_engine.sv tb_dw.sv; vvp /tmp/tb_dw.vvp
echo "== FC classifier tail in HW (global avg-pool -> requant -> FC -> argmax) =="
iverilog -g2012 -o /tmp/tb_fc.vvp mac_int8.sv mac_array_8x8.sv gemm_top_bram.sv requant_unit.sv fc_engine.sv tb_fc.sv; vvp /tmp/tb_fc.vvp
echo "== full multi-layer sequencer (conv/dw/pw/fc dispatch, whole DS-CNN chain) =="
iverilog -g2012 -o /tmp/tb_seq.vvp mac_int8.sv mac_array_8x8.sv gemm_top_bram.sv im2col_gen.sv requant_unit.sv layer_engine.sv dw_engine.sv fc_engine.sv dscnn_seq.sv tb_seq.sv; vvp /tmp/tb_seq.vvp
